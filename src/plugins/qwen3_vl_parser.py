...
Uses Qwen3 VL via SiliconFlow API for document parsing and OCR.
"""
...
class Qwen3VLParser(BaseDocumentProcessor):
    """Qwen3 VL Parser using SiliconFlow API"""
    ...
    def get_supported_extensions(self) -> list[str]:
        """Qwen3 VL supports PDF and images"""
    ...
    def check_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                return {
                    "status": "healthy",
                    "message": "Qwen3 VL (SiliconFlow) is available",
    ...
    def process_file(self, file_path: str, params: dict[str, Any] | None = None) -> str:
        """
        Process file using Qwen3 VL via SiliconFlow
        """
        ...
        try:
            ...
            logger.info(f"Qwen3 VL starting: {os.path.basename(file_path)}")
            ...
            logger.info(
                f"Qwen3 VL finished: {os.path.basename(file_path)} - {len(content)} chars ({processing_time:.2f}s)"
            )
            ...
            if isinstance(e, DocumentParserException):
                raise
            error_msg = f"Qwen3 VL failed: {str(e)}"
